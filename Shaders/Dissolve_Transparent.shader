Shader "Custom/Dissolve/Height/Transparent" {
	Properties{
	...
	SubShader{
		...
		Pass{ 
			...
			{
				float l = _Height - i.worldPos.y;
				float _AlphaTest = sign(i.worldPos.y) * sign(i.worldPos.y) * (l + (tex2D(_DissTexture, i.uv) * _Interpolation));
				clip(_AlphaTest);
		...
		void surf(Input IN, inout SurfaceOutputStandard o) {
			...
			float _AlphaTest = sign(IN.worldPos.y) * sign(IN.worldPos.y) * (l + (tex2D(_DissTexture, IN.uv_DissTexture) * _Interpolation));
			clip(_AlphaTest);
