...
namespace Firebase.Sample.Auth
{
    ...
    public class FirebaseLogin_Sample : MonoBehaviour
    {
        ...
        protected void InitializeFirebase()
        {
                  ...
                  String.IsNullOrEmpty(otherAuthOptions.ProjectId)))
            {
                try
                {
                    otherAuth = FirebaseAuth.GetAuth(FirebaseApp.Create(otherAuthOptions, "Secondary"));
        ...
        protected Task UnlinkEmailAsync()
        {
            ...
            DisableUI();
            return auth.CurrentUser.UnlinkAsync( EmailAuthProvider.GetCredential(email, password).Provider)
                .ContinueWithOnMainThread(task =>
                {
                    EnableUI();
        ...
        protected FederatedOAuthProvider BuildFederatedOAuthProvider(string providerId)
        {
            FederatedOAuthProviderData data = new FederatedOAuthProviderData();
        ...
        {
            FederatedOAuthProvider provider = BuildFederatedOAuthProvider(providerId);
        ...
        {
            var phoneAuthProvider = PhoneAuthProvider.GetInstance(auth);
        ...
        void GUIDisplayGameCenterControls()
        {
            ...
            if (isOnIosDevice || isOnOSXDesktop)
            {
                ...
                bool gameCenterEnabled = (isOnIosDevice ?
                                            GameCenterAuthProvider.IsPlayerAuthenticated() :
                                            false);
        ...
        void GUIDisplayControls()
        {
            if (UIEnabled)
            {
                ...
                {
                    SignInWithProvider(MicrosoftAuthProvider.ProviderId);
                }
                if (GUILayout.Button("SignInWith | Yahoo"))
                {
                    SignInWithProvider(YahooAuthProvider.ProviderId);
                }
                if (GUILayout.Button("ReauthWith | Microsoft"))
                {
                    ReauthenticateWithProvider(MicrosoftAuthProvider.ProviderId);
                }
                if (GUILayout.Button("ReauthWith | Yahoo"))
                {
                    ReauthenticateWithProvider(YahooAuthProvider.ProviderId);
                }
                if (GUILayout.Button("LinkWith | Microsoft"))
                {
                    LinkWithProvider(MicrosoftAuthProvider.ProviderId);
                }
                if (GUILayout.Button("LinkWith | Yahoo"))
                {
                    LinkWithProvider(YahooAuthProvider.ProviderId);
                }
                if (GUILayout.Button("Unlink User | Microsoft"))
                {
                    UnlinkUser(MicrosoftAuthProvider.ProviderId);
                }
                if (GUILayout.Button("Unlink User | Yahoo"))
                {
                    UnlinkUser(YahooAuthProvider.ProviderId);
                }
