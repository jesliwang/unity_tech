...
public class AWSUnitySDKVersionChecker : AssetPostprocessor
{
    ...
    private static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromPath)
    {
        foreach (string assetPath in importedAssets)
        {
            ...
            if (!string.IsNullOrEmpty(ext) && ext.ToLowerInvariant() == ".txt" && assetPath.Contains(SDK_VERSION_PREFIX))
            {                     
                try
                {
                ...
                foreach (string dir in resourceDirs)
                {
                    ...
                    foreach (string fileName in fileEntries)
                    {
                        ...
                        if (!string.IsNullOrEmpty(ext) && ext.ToLowerInvariant() == ".txt" && fileName.Contains(SDK_VERSION_PREFIX))
                        {
                            try
                            {
                                // get version string
    ...
    private static string GetComponentName(string filename)
    {
        ...
        int pos1 = filename.ToLowerInvariant().LastIndexOf(SDK_VERSION_PREFIX);
        int pos2 = filename.ToLowerInvariant().LastIndexOf(".txt");
