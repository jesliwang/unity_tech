...
namespace Spine.Unity.Editor {
    public static class SkeletonBaker
    {
		...
		if (skeletonDataAsset.controller != null) {
            ...
            {
                //NDebug.Log("({0}){1} : {3} + {2} + {4}", o.GetType(), o.name, o.hideFlags, o.GetInstanceID(), o.GetHashCode());
                ...
                if (o is AnimationClip)
                {
                    ...
                    if (!clip.HasFlag(HideFlags.HideInHierarchy))
                    {
                        ...
                        {
                            NDebug.LogWarning("Duplicate AnimationClips were found named {0}", clip.name);
                        }
        ...
        {
#if UNITY_5 || UNITY_2017_1_OR_NEWER
            AnimationUtility.SetAnimationClipSettings(clip, settings.Get());
#else
